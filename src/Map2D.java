 ...
 * 1. init, copy, rotate (left or right)
 * 2. set the map to be cyclic or not cyclic.
 * 3. shortest path (obstacle avoiding) between two points.
 * 4. shortest path (obstacle avoiding) between a set of points (aka TSP problem).
...
public interface Map2D {
	...
	/**
	 * Compute a path (the shorter, the better) visiting all the points (in any order).
	 * Denote: this problem addresses the known TSP problem which is known to be N.P. Complete, see: https://en.wikipedia.org/wiki/Travelling_salesman_problem
	 * A valid path needs to "visit" all the points (in any order) without containing any pixel with the absColor.
	 * A path is an ordered set of pixels where each consecutive pixels in the path are neighbors in this map.
	 * Two pixels are neighbors in the map, iff they are a single pixel apart (up,down, left, right).
	 * In case there is no valid path between p1 and p2 should return null;
	 * If this map is cyclic:
	 ...
	 * 4. the pixel below (j,0) is (j,getHeight()-1).
	 * Where 0<=i<getWidth(), 0<=j<getWidth().
	 * In case points is NOT valid (contains less than 2 points or there is no such path) should return null.
	 *
